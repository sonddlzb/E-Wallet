...
class SeekBarView: UIView {
    ...
    private func handleSeeking(touch: UITouch) {
        let positionX = min(touch.location(in: self).x, self.frame.width - self.circleView.frame.width/2)
        self.timeFillViewTrailingConstraint.constant = positionX
        self.layoutIfNeeded()
        if self.timeNotFillView.bounds.width != 0.0 {
            delegate?.seekBarView(self, seekTimeProgress: positionX/self.timeNotFillView.bounds.width)
        }
    }
}
